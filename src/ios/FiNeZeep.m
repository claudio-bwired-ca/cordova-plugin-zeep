...
-(void)zip:(CDVInvokedUrlCommand*)command
{
    ...
    @try
    {
        ...
        if([SSZeepArchive createZipFileAtPath:toPath withContentsOfDirectory:fromPath keepParentDirectory:NO withPassword:nil])
        {
...
-(void)unzip:(CDVInvokedUrlCommand*)command
{
    ...
    @try
    {
        ...
        if([SSZeepArchive unzipFileAtPath:fromPath toDestination:toPath overwrite:YES password:nil error:&error delegate:self])
        {
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
        }
        else
